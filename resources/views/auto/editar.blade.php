        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="backdrop-blur-lg mx-auto bg-opacity-80 rounded-lg shadow-lg p-5 bg-gray-900 text-white">
                    ...
                    <form action="{{ route('auto.update', ['id' => $auto->id]) }}" method="POST" autocomplete="off"
                            ...
                            <select id="titular_id" name="titular_id"
                                ...
                                @foreach ($titulares as $titular)
                                    <option value="{{ $titular->id }}"
                                        @if ($titular->id == $titularPorDefecto->id) selected @endif>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="tipo" id="tipo" name="tipo"
                                ...
                                @foreach ($tiposAutos as $tipo)
                                    <option value="{{ $tipo }}"
                                        @if ($tipo == $auto->tipo) selected @endif>{{ strtoupper($tipo) }}
                                    </option>
                                @endforeach
                            </select>
                            @error('tipo')
