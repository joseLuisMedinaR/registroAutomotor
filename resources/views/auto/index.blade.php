...
<x-app-layout>
    ...
    <div class="py-12">
                    ...
                    <table id="example"
                        ...
                        <thead>
                            <tr class="bg-gray-100">
                                ...
                                <th class="px-4 py-2 p-3 text-sm tracking-wide text-left"># ID Titular</th>
                                <th class="px-4 py-2 p-3 text-sm tracking-wide text-left">Titular</th>
                                <th class="px-4 py-2 p-3 text-sm tracking-wide text-left">Marca</th>
                                <th class="px-4 py-2 p-3 text-sm tracking-wide text-left">Modelo</th>
                        ...
                        <tbody>
                            @foreach ($auto as $autos)
                                <tr>
                                    <td class="border px-4 py-2 p-3 text-sm tracking-wide text-left">{{ $autos->id }}
                                    </td>
                                    <td class="border px-4 py-2 p-3 text-sm tracking-wide text-left">
                                        {{ $autos->titular_id }}</td>
                                    <td class="border px-4 py-2 p-3 text-sm tracking-wide text-left">
                                        {{ ucwords($autos->titular->apellido) . ', ' . ucwords($autos->titular->nombre) }}
                                    </td>
                                    <td class="border px-4 py-2 p-3 text-sm tracking-wide text-left">
                                        {{ ucwords($autos->marca) }}</td>
                                    <td class="border px-4 py-2 p-3 text-sm tracking-wide text-left">
                                        {{ ucwords($autos->modelo) }}</td>
                                    <td class="border px-4 py-2 p-3 text-sm tracking-wide text-left">
                                        {{ strtoupper($autos->patente) }}</td>
                                    <td class="border px-4 py-2 p-3 text-sm tracking-wide text-left">
                                        {{ strtoupper($autos->tipo) }}</td>
                                    <td class="border px-4 py-2 p-3 text-sm tracking-wide text-left"
                                        style="width: 10px">
                                        <div class="flex justify-center items-center rounded-full mx-auto"
                                            style="width: fit-content; padding: 0 12px;">
                                            <div class="mr-2">
                                                <a href="{{ route('auto.show', $autos->id) }}">
                                                    <button
                                                        class="font-bold md:text-xl bg-green-500 rounded-full text-white flex items-center px-4 py-1 hover:bg-lime-600">
                                            ...
                                            <div class="mx-2">
                                                <a href="{{ route('auto.edit', $autos->id) }}">
                                                    <button
                                                        class="font-bold md:text-xl bg-blue-600 rounded-full text-white flex items-center px-4 py-1 hover:bg-blue-800">
                                ...
                                <div id="modelConfirm" class="fixed hidden z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4 ">
                                    <div class="relative top-40 mx-auto shadow-xl rounded-md bg-white max-w-md">
                                        ...
                                        <div class="p-6 pt-0 text-center">
                                            ...
                                            <a href="" onclick="deleteAuto('{{ $autos->id }}', 'modelConfirm')" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2">
                                                Si, eliminar
                                            </a>                                            
     ...
     <script type="text/javascript">
         ...
         function deleteAuto(autoId, modalId) {
    fetch('{{ route("auto.destroy", ":id") }}'.replace(':id', autoId), {
        method: 'DELETE',
        headers: {
    ...
    .then(response => {
        if (response.ok) {
            // Si la eliminación se realiza correctamente, cierra el modal y actualiza la tabla de autos
            closeModal(modalId); // Aquí se cierra el modal
            window.location.reload(); // Esto recarga la página, considera otro enfoque si prefieres actualizar solo la tabla
            document.getElementById(modalId).style.display = 'none'
             document.getElementsByTagName('body')[0].classList.remove('overflow-y-hidden')
        } else {
