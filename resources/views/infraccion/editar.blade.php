        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="backdrop-blur-lg mx-auto bg-opacity-80 rounded-lg shadow-lg p-5 bg-gray-900 text-white">
                    ...
                    <form action="{{ route('infraccion.update', ['id' => $infraccion->id]) }}" method="POST"
                        autocomplete="off" name="editar" id="editar">
                        @csrf
                        {{ method_field('PUT') }}
                            ...
                            <select id="auto_id" name="auto_id"
                                ...
                                @foreach ($autos as $auto)
                                    <option value="{{ trim($auto->id) }}"
                                        @if ($infraccion->auto_id == $auto->id) selected @endif>
                                        {{ strtoupper($auto->patente) }}
                                    </option>
                                @endforeach
                            </select>
                            @error('auto_id')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="tipo" id="tipo" name="tipo"
                                ...
                                @foreach ($tiposInfracciones as $tipo)
                                    <option value="{{ $tipo }}"
                                        @if ($tipo == $infraccion->tipo) selected @endif>{{ strtoupper($tipo) }}
                                    </option>
                                @endforeach
                            </select>
                            @error('tipo')
